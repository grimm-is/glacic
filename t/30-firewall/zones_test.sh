...
if ! kill -0 $PID 2>/dev/null; then
    ...
    ok 0 "Firewall started with zone config (PID $PID)"
    diag "Firewall startup logs:"
    cat /tmp/firewall.log
fi
...
ip netns exec green curl -v --max-time 2 -s http://10.1.0.1:8080/api/status > /tmp/api_test.log 2>&1
