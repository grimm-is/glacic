...
# Check Sync
grep "Connected to primary" $REPL_DIR/log >/dev/null 2>&1
res=$?
if [ $res -ne 0 ]; then
    echo "### FAILURE DEBUG - REPLICA LOG:"
    cat $REPL_DIR/log
    echo "### FAILURE DEBUG - PRIMARY LOG:"
    cat $PRIM_DIR/log
    echo "### FAILURE DEBUG - PRIMARY RULESET:"
    ip netns exec ns_prim nft list ruleset
fi
ok $res "Replica connected to Primary"

grep -E "Applied incremental changes|Restored snapshot" $REPL_DIR/log >/dev/null 2>&1
ok $? "Replica received sync data"
...
# Check loaded leases
grep "Loaded .* leases from state store" $REPL_DIR/log2 >/dev/null 2>&1
if [ $? -ne 0 ]; then
    fail "Replica failed to load state from DB"
fi
