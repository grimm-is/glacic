...
diag "Testing 'apply --dryrun' command..."

# Scenario 3.1: Dry-run valid config should success and print rules
OUTPUT=$($APP_BIN apply --dryrun "$CONFIG_VALID" 2>&1)
EXIT_CODE=$?

if [ $EXIT_CODE -eq 0 ]; then
    ok 0 "glacic apply --dryrun exit code 0"
else
    ok 1 "glacic apply --dryrun exit code $EXIT_CODE"
fi

if echo "$OUTPUT" | grep -q "flush ruleset"; then
    ok 0 "glacic apply --dryrun generated rules"
else
    ok 1 "glacic apply --dryrun did not generate expected rules"
fi
