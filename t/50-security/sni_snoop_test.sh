...
wait_for_port 8093 10 || fail "API failed to start"

# Test 1: SNI data endpoint
diag "Test 1: SNI data endpoint"
response=$(curl -s -o /dev/null -w "%{http_code}" "http://169.254.255.2:8093/api/flows/sni" 2>&1)
pass "SNI flows endpoint accessible (status: $response)"

# Test 2: Connections with SNI
diag "Test 2: Connections endpoint"
response=$(curl -s -o /dev/null -w "%{http_code}" "http://169.254.255.2:8093/api/flows" 2>&1)
