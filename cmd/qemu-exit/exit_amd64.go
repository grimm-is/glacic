...
func qemuExit(exitCode int) {
	...
	if err == nil {
		...
		_, err = f.Seek(0xf4, 0)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Failed to seek to port: %v\n", err)
			fallbackPoweroff()
			return
		}
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "Failed to write to port: %v\n", err)
			fallbackPoweroff()
		}
	...
	fmt.Fprintf(os.Stderr, "Cannot access /dev/port: %v, falling back to poweroff\n", err)
	fallbackPoweroff()
}
