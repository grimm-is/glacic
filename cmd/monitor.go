...
func RunMonitor(args []string) {
	fmt.Println("Starting Glacic Self-Healing Monitor...")
	...
	for {
		...
		if err := cmd.Start(); err != nil {
			fmt.Fprintf(os.Stderr, "Monitor: Failed to start child: %v\n", err)
			time.Sleep(2 * time.Second)
			continue
		}

		fmt.Printf("Monitor: Started glacic ctl (pid=%d)\n", cmd.Process.Pid)
		...
		go func() {
			...
			case sig := <-sigCh:
				...
				if cmd.Process != nil {
					fmt.Printf("Monitor: Forwarding signal %v to child...\n", sig)
		...
		// Analyze exit
		if err == nil {
			fmt.Println("Monitor: Child exited cleanly. Shutting down monitor.")
			os.Exit(0)
		}
		...
		if exitCode == 0 {
			fmt.Println("Monitor: Child exited with code 0. Shutting down monitor.")
			os.Exit(0)
		}

		fmt.Printf("Monitor: Child crashed with exit code %d (err: %v). Restarting...\n", exitCode, err)
		...
		if fastCrashCount >= maxFastCrashes {
			fmt.Printf("Monitor: Detected crash loop (%d fast crashes). Backing off for 30s...\n", fastCrashCount)
