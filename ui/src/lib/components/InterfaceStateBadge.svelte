<script lang="ts">
  ...
  const stateConfig: Record<
    string,
    { label: string; color: string; iconName: string; filled: boolean }
  > = {
    up: { label: "Up", color: "success", iconName: "state-up", filled: true },
    down: {
      label: "Down",
      color: "neutral",
      iconName: "state-down",
...
<span
  ...
  role="status"
  aria-label="Interface state: {config.label}"
>
  <Icon name={config.iconName} {size} filled={config.filled} />
  {#if showLabel}
    <span class="state-badge__label">{config.label}</span>
  {/if}
</span>
