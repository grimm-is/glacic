...
<div class="routing-page">
  ...
  <div class="tabs">
    <Button
      ...
      onclick={() => (activeTab = "routes")}>Static Routes</Button
    >
    <Button
      variant={activeTab === "marks" ? "default" : "ghost"}
      onclick={() => (activeTab = "marks")}>Mark Rules</Button
    >
    <Button
      variant={activeTab === "uid" ? "default" : "ghost"}
      onclick={() => (activeTab = "uid")}>User Routing</Button
    >
  </div>
  ...
  {#if activeTab === "routes"}
    <div class="sub-header">
      <h3>Static Routes</h3>
      <Button onclick={openAddRoute} size="sm">+ Add Route</Button>
    </div>
    <Card>
      {#if routes.length === 0}
        <p class="empty-message">No static routes configured.</p>
      {:else}
        <div class="routes-list">
          {#each routes as route, index}
            <div class="route-row">
              <code class="route-dest">{route.destination}</code>
              <span class="route-arrow">via</span>
              {#if route.gateway}
              ...
              {/if}
              <span class="route-metric">metric {route.metric || 100}</span>
              <Button
                variant="ghost"
                size="sm"
  ...
  {:else if activeTab === "marks"}
    <div class="sub-header">
      <h3>Mark Rules</h3>
      <Button onclick={openAddMarkRule} size="sm">+ Add Mark Rule</Button>
    </div>
    <Card>
      {#if markRules.length === 0}
        <p class="empty-message">No mark rules configured.</p>
      {:else}
        <Table
          columns={[
            { key: "name", label: "Name" },
          ...
          {#snippet row(r, i)}
            ...
            <td>
              <div class="filter-match">
                {#if r.src_ip}
                  <span>Src: {r.src_ip}</span>
                {/if}
                {#if r.out_interface}
                  <span>Out: {r.out_interface}</span>
                {/if}
              </div>
            </td>
            <td>{r.save_mark ? "Save Mark" : ""}</td>
            <td class="actions">
              <Button
                variant="ghost"
                size="sm"
  ...
  {:else if activeTab === "uid"}
    <div class="sub-header">
      <h3>UID Routing</h3>
      <Button onclick={openAddUIDRule} size="sm">+ Add UID Rule</Button>
    </div>
    <Card>
      {#if uidRouting.length === 0}
        <p class="empty-message">No UID routing rules configured.</p>
      {:else}
        <div class="routes-list">
          {#each uidRouting as route, index}
            <div class="route-row">
              <span>UID: <b>{route.uid}</b></span>
              <span class="route-arrow">via</span>
...
<Modal
  ...
  title={isEditMode ? "Edit Mark Rule" : "Add Mark Rule"}
>
  <div class="form-stack">
    <Input id="mr-name" label="Name" bind:value={mrName} required />
    <Input
      id="mr-mark"
      label="Mark (Int)"
      bind:value={mrMark}
      ...
      required
    />
    <Input id="mr-src" label="Source IP" bind:value={mrSrcIP} />
    <Select
      id="mr-iface"
      label="Out Interface"
    ...
    <div class="modal-actions">
      <Button variant="ghost" onclick={() => (showMarkModal = false)}
        >Cancel</Button
      >
      <Button onclick={saveMarkRule} disabled={loading}>Save</Button>
    </div>
  </div>
</Modal>
...
<Modal
  ...
  title={isEditMode ? "Edit UID Rule" : "Add UID Rule"}
>
  <div class="form-stack">
    <Input id="uid-name" label="Name" bind:value={uidName} required />
    <Input
      id="uid-uid"
      label="UID"
      bind:value={uidUID}
    ...
    <div class="modal-actions">
      <Button variant="ghost" onclick={() => (showUIDModal = false)}
        >Cancel</Button
      >
      <Button onclick={saveUIDRule} disabled={loading}>Save</Button>
    </div>
  </div>
</Modal>
...
<Modal
  ...
  title={isEditMode ? "Edit Static Route" : "Add Static Route"}
>
  <div class="form-stack">
    <Input
      id="route-dest"
