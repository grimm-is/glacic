...
func (s *Server) handleSystemReboot(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "Failed to reboot system: "+err.Error())
		return
	}
...
func (s *Server) handleSystemUpgrade(w http.ResponseWriter, r *http.Request) {
	...
	if !strings.HasPrefix(contentType, "multipart/form-data") {
		...
		if err := s.client.Upgrade(""); err != nil {
			WriteError(w, http.StatusInternalServerError, "Upgrade failed: "+err.Error())
			return
		}
		WriteJSON(w, http.StatusOK, map[string]interface{}{
			"success": true,
	...
	if err != nil {
		WriteError(w, http.StatusBadRequest, "Missing 'binary' field: "+err.Error())
		return
	}
	defer file.Close()
	...
	if expectedChecksum == "" {
		WriteError(w, http.StatusBadRequest, "Missing 'checksum' field")
		return
	}
	...
	if expectedArch == "" {
		WriteError(w, http.StatusBadRequest, "Missing 'arch' field")
		return
	}
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "Failed to read binary: "+err.Error())
		return
	}
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "Failed to stage binary: "+err.Error())
		return
	}
...
func (s *Server) handleWakeOnLAN(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.client.WakeOnLAN(req.MAC, req.Interface); err != nil {
		WriteError(w, http.StatusInternalServerError, "Failed to send WOL packet: "+err.Error())
		return
	}
...
func (s *Server) handleSystemRoutes(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		WriteError(w, http.StatusInternalServerError, "Failed to get routes: "+err.Error())
		return
	}
