...
func (g *UplinkGroup) SwitchTo(uplink *Uplink) error {
	...
	for _, srcNet := range g.SourceNetworks {
		if err := g.updateNewConnectionMark(srcNet, newMark); err != nil {
...
func (g *UplinkGroup) Setup() error {
	...
	for _, srcNet := range sourceNets {
		if len(weightMap) > 1 {
			...
			if err := g.updateNewConnectionMark(srcNet, currentMark); err != nil {
...
func (h *UplinkHealthChecker) checkGroup(group *UplinkGroup) {
	...
	if bestTier >= 0 && bestTier < currentTier && failbackMode != FailbackNever {
		tierChanged = true
	}
