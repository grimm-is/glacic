	...
	for {
		...
		if err == nil {
			if err := m.applyDHCPLease(ifaceName, lease, tableID); err != nil {
				...
				// But runDHCPClientLoop starts with Request() which is DORA.
				// We need separate Renew loop.
				// Actually, nclient4 example shows just usage.
				return // TODO: Need cleaner state machine.
			}
		}
