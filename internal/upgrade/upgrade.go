...
func (m *Manager) RegisterListener(name string, listener net.Listener) {
	...
	m.logger.Info("Registered listener for upgrade handoff", "name", name)
}
...
func (m *Manager) handoffListeners(ctx context.Context, conn net.Conn) error {
	...
	for name, listener := range m.listeners {
		...
		m.logger.Info("Handed off listener", "name", name)
	}
...
// getListenerFile extracts the file descriptor from a listener.
	...
	for {
		...
		for _, scm := range scms {
			...
			for _, fd := range fds {
				...
				if err != nil {
					m.logger.Warn("Failed to create listener from fd", "name", name, "error", err)
					continue
				}
				...
				m.logger.Info("Received listener", "name", name, "addr", listener.Addr())
			}
		}
	}
}
