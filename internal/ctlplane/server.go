...
func (s *Server) GetTopology(args *Empty, reply *GetTopologyReply) error {
	if s.lldpService == nil {
		// Just return empty if not running, or error?
		// Empty is safer for UI
	...
	for i, n := range neighbors {
		if n.Info == nil {
			// Skip malformed entries or provide defaults
...
func (s *Server) reloadConfigInternal(newCfg *config.Config) error {
	...
	if err := s.uplinkManager.Reload(newCfg.UplinkGroups); err != nil {
		...
		// Start health checking with defaults (Google/Cloudflare DNS)
...
func (s *Server) syncScheduledRules(cfg *config.Config) error {
	...
	for _, task := range currentTasks {
		if strings.HasPrefix(task.ID, "rule_") {
			...
			// TODO: Optimize to only remove changed ones?
			// For now, simple wipe and recreate is safer to ensure state consistency
