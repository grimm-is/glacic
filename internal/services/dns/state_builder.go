...
func (s *Service) buildServerState(cfg *config.Config) *serverState {
	...
	for _, addr := range listenAddrs {
		// Port 53 hardcoded for now as per original logic
		state.servers = append(state.servers, &dns.Server{Addr: net.JoinHostPort(addr, "53"), Net: "udp", Handler: s})
		state.servers = append(state.servers, &dns.Server{Addr: net.JoinHostPort(addr, "53"), Net: "tcp", Handler: s})
	}
