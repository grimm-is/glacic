...
func matchesZone(pattern, zoneName string) bool {
	...
	matched, _ := filepath.Match(pattern, zoneName)
	return matched
}
...
func (c *Config) NormalizeZoneMappings() {
	// Build map of zone name -> zone pointer
	...
	for _, zone := range c.Zones {
		for _, ifaceName := range zone.Interfaces {
			if iface, exists := ifaceMap[ifaceName]; exists {
				// If interface doesn't have a zone set, or has a different zone (conflict?)
				// We prioritize the zone definition if iface.Zone is empty.
				// If both are set and different, usually last write wins or we prefer explicit interface config.
				// Here we'll set it if empty to support the "new" syntax.
				if iface.Zone == "" {
					iface.Zone = zone.Name
				}
			} else {
				// Implicit interface definition? Maybe warned about validation elsewhere.
			}
		}
	}
	...
	for _, iface := range c.Interfaces {
		if iface.Zone != "" {
			zone, exists := zoneMap[iface.Zone]
			if !exists {
				...
				// Re-update map reference might be tricky with append reallocating,
				// but since we only append, we should be careful.
				// Safest is to just re-fetch or use index if we weren't using pointers.
				// Since we are iterating and appending, it's safer to re-lookup or just update the map correctly.
				// However, simplistic append might invalidate pointers in zoneMap if capacity is exceeded.
				// Let's rebuild the map entry or just append carefully.
				// Actually, simpler: we just need to ensure the zone exists.
				// We can't rely on pointers to slice elements being stable across appends.
				// So we should re-derive pointers or use indices. Used implicit logic before.
				zoneMap[iface.Zone] = &c.Zones[len(c.Zones)-1]
			} else {
		...
		for _, vlan := range iface.VLANs {
			if vlan.Zone != "" {
				...
				if !exists {
					...
					c.Zones = append(c.Zones, newZone)
					zoneMap[vlan.Zone] = &c.Zones[len(c.Zones)-1]
				} else {
