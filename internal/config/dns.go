...
// MigrateDNSConfig migrates legacy dns_server{} config to new dns{} structure.
// This maintains backwards compatibility with existing configs.
func (c *Config) MigrateDNSConfig() {
	if c.DNSServer == nil {
		return
	}
	...
	if c.DNSServer.Enabled {
		// Create a "legacy" serve block that covers all zones
		// Using "*" as a wildcard to match all zones
		serve := DNSServe{
			Zone:             "*",
