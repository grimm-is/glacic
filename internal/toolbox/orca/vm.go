...
func (v *VM) Start(ctx context.Context) error {
	...
	createCmd := exec.Command("qemu-img", "create", "-f", "qcow2", "-b", v.Config.RootfsPath, "-F", "qcow2", overlayPath)
	if out, err := createCmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed to create overlay: %v (%s)", err, out)
	}
	...
	args := []string{
		...
		// Host Share
		"-virtfs", fmt.Sprintf("local,path=%s,mount_tag=host_share,security_model=none,id=host_share", v.Config.ProjectRoot),
	}
	...
	v.cmd = exec.CommandContext(ctx, qemuBin, args...)
